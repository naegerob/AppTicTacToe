...
class MainActivity : ComponentActivity() {
                                ...
                                for (j in 0..2) {
                                    ...
                                    Button(modifier = buttonModifier, colors = ButtonDefaults.buttonColors(backgroundColor = buttonColor),
                                        onClick = {
                                            ...
                                            numberOfStepsPlayed++
                                            updateButton(buttonId, buttonTextList, numberOfStepsPlayed)
                                            ...
                                            else if(gameAgainstHuman and checkResult(buttonTextList, Player.Player2.symbol)) {
                                            ...
                                            if(!gameAgainstHuman and (whichPlayerClicked(numberOfStepsPlayed) == Player.Player1)) {
                                                numberOfStepsPlayed++
                                                runBlocking(Dispatchers.IO) { // equivalent as updateButton for human
                                                    buttonTextList = getNPCMove(client, buttonTextList)
                                                }
                                                if(checkResult(buttonTextList, Player.Player2.symbol)) {
                                                    buttonColor = Color.Green
                                                    isGameWon = true
                                                    buttonTextList.clear()
                                                    buttonTextList.addAll(List(9) { "" })
    ...
    private fun updateButton(button: Int, buttonTextList : MutableList<String>, counter: Int) {
        if(button in 0..8) {
            buttonTextList[button] = if (whichPlayerClicked(counter) == Player.Player2) Player.Player2.symbol else Player.Player1.symbol
        }
    }
    ...
    suspend fun getNPCMove(client: HttpClient, gameField: SnapshotStateList<String>): SnapshotStateList<String> {
